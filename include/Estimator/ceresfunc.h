...
struct Cost_NavState_PRV_Bias
{
	Cost_NavState_PRV_Bias(IMUIntegrator &measure_,
						   ...
						   Eigen::Matrix<double, 15, 15> sqrt_information_) : imu_measure(measure_),
...
struct Cost_NavState_IMU_Line
{
	Cost_NavState_IMU_Line(Eigen::Vector3d _p, Eigen::Vector3d _vtx1, Eigen::Vector3d _vtx2,
						   const Eigen::Matrix4d &Tbl, Eigen::Matrix<double, 1, 1> sqrt_information_) : point(std::move(_p)), vtx1(std::move(_vtx1)), vtx2(std::move(_vtx2)),
																										sqrt_information(std::move(sqrt_information_))
	{
	...
	bool operator()(const T *PRi, T *residual) const
	{
		...
		T a012 = ceres::sqrt(
			((P_to_Map(0) - lpa(0)) * (P_to_Map(1) - lpb(1)) - (P_to_Map(0) - lpb(0)) * (P_to_Map(1) - lpa(1))) * ((P_to_Map(0) - lpa(0)) * (P_to_Map(1) - lpb(1)) - (P_to_Map(0) - lpb(0)) * (P_to_Map(1) - lpa(1))) + ((P_to_Map(0) - lpa(0)) * (P_to_Map(2) - lpb(2)) - (P_to_Map(0) - lpb(0)) * (P_to_Map(2) - lpa(2))) * ((P_to_Map(0) - lpa(0)) * (P_to_Map(2) - lpb(2)) - (P_to_Map(0) - lpb(0)) * (P_to_Map(2) - lpa(2))) + ((P_to_Map(1) - lpa(1)) * (P_to_Map(2) - lpb(2)) - (P_to_Map(1) - lpb(1)) * (P_to_Map(2) - lpa(2))) * ((P_to_Map(1) - lpa(1)) * (P_to_Map(2) - lpb(2)) - (P_to_Map(1) - lpb(1)) * (P_to_Map(2) - lpa(2))));
		T ld2 = a012 / T(l12);
		T _weight = T(1) - T(0.9) * ceres::abs(ld2) / ceres::sqrt(ceres::sqrt(P_to_Map(0) * P_to_Map(0) + P_to_Map(1) * P_to_Map(1) + P_to_Map(2) * P_to_Map(2)));
...
struct Cost_NavState_IMU_Plan
{
	Cost_NavState_IMU_Plan(Eigen::Vector3d _p, double _pa, double _pb, double _pc, double _pd,
						   ...
						   const Eigen::Matrix4d &Tbl, Eigen::Matrix<double, 1, 1> sqrt_information_) : point(std::move(_p)), pa(_pa), pb(_pb), pc(_pc), pd(_pd), sqrt_information(std::move(sqrt_information_))
	{
		Eigen::Matrix3d m3d = Tbl.topLeftCorner(3, 3);
		qbl = Eigen::Quaterniond(m3d).normalized();
		qlb = qbl.conjugate();
	...
	bool operator()(const T *PRi, T *residual) const
	{
		...
		T _weight = T(1) - T(0.9) * ceres::abs(pd2) / ceres::sqrt(ceres::sqrt(P_to_Map(0) * P_to_Map(0) + P_to_Map(1) * P_to_Map(1) + P_to_Map(2) * P_to_Map(2)));
...
struct Cost_NavState_IMU_Plan_Vec
{
	Cost_NavState_IMU_Plan_Vec(Eigen::Vector3d _p,
							   ...
							   const Eigen::Matrix4d &Tbl,
							   Eigen::Matrix<double, 3, 3> _sqrt_information) : point(std::move(_p)),
	...
	bool operator()(const T *PRi, T *residual) const
	{
		...
		T _weight = T(1) - T(0.9) * (P_to_Map - cp_proj).norm() / ceres::sqrt(ceres::sqrt(P_to_Map(0) * P_to_Map(0) + P_to_Map(1) * P_to_Map(1) + P_to_Map(2) * P_to_Map(2)));
		eResiduals *= _weight;
		eResiduals.applyOnTheLeft(sqrt_information.template cast<T>());
...
struct Cost_Initialization_IMU
{
	...
	Cost_Initialization_IMU(IMUIntegrator &measure_,
							...
							Eigen::Vector3d rj_,
							Eigen::Vector3d dp_,
							Eigen::Matrix<double, 9, 9> sqrt_information_) : imu_measure(measure_),
																			 ri(ri_),
																			 rj(rj_),
																			 dp(dp_),
																			 sqrt_information(std::move(sqrt_information_)) {}
...
struct Cost_Initialization_Prior_bv
{
	...
	Cost_Initialization_Prior_bv(Eigen::Vector3d prior_,
								 Eigen::Matrix3d sqrt_information_) : prior(prior_),
																	  sqrt_information(std::move(sqrt_information_)) {}
